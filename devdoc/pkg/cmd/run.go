...
// runServe is the entry point for `devdoc run` (and its `serve` alias).
// It does the first build synchronously, starts the HTTP server, then
// runs the watcher in the foreground. Each successful rebuild calls
// Server.Reload() so the browser knows to refresh.
func runServe(dir, dest, hostname, port string) error {
