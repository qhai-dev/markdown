...
// error path through the load pipeline (Load → read devdoc.yaml → open file).
func TestFormatErrorChain(t *testing.T) {
	...
	mid := fmt.Errorf("open /tmp/foo/devdoc.yaml: %w", inner)
	top := fmt.Errorf("read book config: %w", mid)

	got := formatError(top)
	want := "read book config: open /tmp/foo/devdoc.yaml: no such file or directory\n" +
		"\tCaused by: open /tmp/foo/devdoc.yaml: no such file or directory\n" +
